...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <button onclick="loadProDetail(${product.id})" id="details-btn" class="btn btn-danger">Details</button></div>
      `;
...
const loadProDetail = productId => {
  //console.log(productId);
  const url = `https://fakestoreapi.com/products/${productId}`
  fetch(url)
    .then(res => res.json())
    .then(data => displayProDetail(data))
}
const displayProDetail = product => {
  console.log(product);
